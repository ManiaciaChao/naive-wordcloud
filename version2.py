...
class Download:
	...
	def download_url(self):
		...
		html=response.text.encode("latin1").decode("gbk")               #将网页文本内容转换为latin1编码
...
if __name__=='__main__':                                                #在当前模块内执行函数
	...
	for i in range(download1.chapters_number):
		if i<12:
			download1.writer(download1.chapters_name[i],download1.download_content1(download1.chapters_url[i]),'平凡的世界.txt')
			sys.stdout.write("已下载：%.3f%%" % float(100*((i+1)/download1.chapters_number))+'\r')
                                                                        #将光标的位置退到本行，标准化输出
		else:
			download1.writer(download1.chapters_name[i],download1.download_content2(download1.chapters_url[i]),'平凡的世界.txt')
			sys.stdout.write("已下载：%.3f%%" % float(100*((i+1)/download1.chapters_number))+ '\r')
		sys.stdout.flush
	print('\n下载完成')                                                  #让缓冲区的内容输出
...
def stopwordslist(filepath):
	stopwordslist=[line.strip() for line in open(filepath,'r',encoding='utf-8').readlines()]
...
def create_wordcloud(filename):
	...
	color_list=['#87CEFA','#0000CD','#ADD8E6','#1E90FF','#00BFFF']      #设置颜色格式
...
def create_relationship(filename,Names):
	...
	relations={namepair:appearance_times/basic for namepair,appearance_times in relations.items()}
	...
	#筛选权重大于0.2小于0.7的边
	emidle=[(u,v) for (u,v,d) in G.edges(data=True) if (d['weight']>0.2) & (d['weight']<=0.7)]
	#筛选权重大于0小于0.2的边
	esmall=[(u,v) for (u,v,d) in G.edges(data=True) if (d['weight']<=0.2)&(d['weight']>0)]
	#设置图形格式
	pos=nx.spring_layout(G)
	#设置节点样式
	nx.draw_networkx_nodes(G,pos,node_color='lightcyan',alpha=0.8, node_size=1200) 
	#设置权重大于0.7的边的格式
	nx.draw_networkx_edges(G,pos,edgelist=elarge, width=3.0,alpha=0.9,edge_color='darkblue')
	#设置权重在0.2~0.7的边的格式
	nx.draw_networkx_edges(G,pos,edgelist=emidle, width=2.0,alpha=0.6,edge_color='royalblue') 
	#设置权重在0~0.2的边的格式
	nx.draw_networkx_edges(G,pos,edgelist=esmall, width=1.0,alpha=0.4,edge_color='cornflowerblue')
...
def create_character_importance(filename,Names):
	...
	for i in x:
		...
		y.append(i0)
	#设置颜色格式
	colors=['darkblue','blue','royalblue','dodgerblue','deepskyblue','lightskyblue','lightblue','lightcyan']
...
def create_character_wordcloud(filename,Names):
	...
	stopwords=stopwordslist('stop.txt')
	stopwords_set = set(line.strip() for line in open(stopwords,"r", encoding="utf8"))
